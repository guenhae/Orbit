...
/***************************************************************************
  ...
  ------------
    ...
    classes/state/ostream.cpp and classes/state/istream.cpp). The function 
...
public:
	...
	state() :number(NULL_STATE) { index = -1; lowlink = -1; on_stack = false; }
			// default constructor:  initializes this state to be
			//  a null state.

	state(const state& s) :number(s.number) { index = s.index; lowlink = s.lowlink; on_stack = s.on_stack; }
			// copy constructor:  initializes this state to be the
			//  same type and have the same state number as the
			//  parameter state.
			
	state(const int& i) :number(i + 2) { index = -1; lowlink = -1; on_stack = false; }
	...
	{
		number = s.number; index = s.index; lowlink = s.lowlink; on_stack = s.on_stack; return *this;
	}
